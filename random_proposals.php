...
<head>
<title> <?= $site_name; ?> - <?= $lang['random_proposals']['title']; ?> </title>
...
<div class="container-fluid mt-5"> <!-- Container start -->
	<div class="row">
		<div class="col-md-12">
			<center>
        <h1> <?= $lang['random_proposals']['title']; ?> </h1>
        <p class="lead"><?= $lang['random_proposals']['desc']; ?></p>
			</center>
			<hr class="mt-5 pt-2">
		</div>
	</div>
	<div class="row mt-3">
		<div class="col-lg-3 col-md-4 col-sm-12 <?=($lang_dir == "right" ? 'order-2 order-sm-1':'')?>">
			<?php require_once("includes/random_sidebar.php"); ?>
		</div>
		<div class="col-lg-9 col-md-8 col-sm-12 <?=($lang_dir == "right" ? 'order-1 order-sm-2':'')?>">
			<div class="row flex-wrap proposals" id="random_proposals">
			<?php get_random_proposals(); ?>
			</div>
			<div id="wait"></div>
			<br>
			<div class="row justify-content-center mb-5 mt-0"><!-- row justify-content-center Starts -->
			<nav><!-- nav Starts -->
			  <ul class="pagination" id="random_pagination">
			  <?php get_random_pagination(); ?>
			  </ul>
			</nav><!-- nav Ends -->
			</div>		
		</div>
...
function get_random_proposals(){
	...
	url:"random_load",  
	method:"POST",  
	data: sPath+'zAction=get_random_proposals',  
	success:function(data){
	$('#random_proposals').html('');  
	$('#random_proposals').html(data); 
	...
	url:"random_load",  
	method:"POST",  
	data: sPath+'zAction=get_random_pagination',  
	success:function(data){  
	$('#random_pagination').html('');  
	$('#random_pagination').html(data); 
	}  
	});
}
